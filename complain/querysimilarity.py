...
def predict1(given_query,database_query):
	'''
	Sample trial for two mails.The larger % similarity to a particular mail type so assign for the
	same
	'''
	keywords_givenquery,keywords_databasequery=keyword_extractor_text(database_query,given_query)
	givenquery_tfidf,databasequery_tfidf=TFIDF(keywords_givenquery,keywords_databasequery)
	similarity=jaccard_similarity(databasequery_tfidf,givenquery_tfidf)
	return similarity
